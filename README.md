...
This project also features a **Human-in-the-loop AI Assistant** built with FastAPI and Streamlit, allowing users to query data via natural language, review generated Python code, and execute it locally for safe and transparent insights.
...
### ✨ Key Feature: Interactive AI Analytics
Beyond static dashboards, we integrated an AI Data Analyst:
*   **Natural Language to Code:** Generates Pandas and Plotly code from user queries (Powered by LLMs).
*   **Human-in-the-loop:** Code is explicitly held for human review and modification before execution, preventing silent errors and unauthorized data mutation.
*   **Centralized API Logging:** All chat sessions, original AI code, user modifications, and execution results are asynchronously tracked via a dedicated logging API (`JSONL` format) for full traceability.
...
- **Self-service Analytics:** Empower users to perform ad-hoc EDA through a governed AI chatbot interface, bridging the gap between raw data and business users.
...
| **Backend & UI** | **FastAPI** (AI Services, Async Logging), **Streamlit** (Interactive Dashboard) |
...
This project implements a Microservice architecture, decoupling the AI logic (FastAPI) from the user interface (Streamlit) for better performance and non-blocking asynchronous logging.
...
- Environment variables in a **`.env`** file (e.g. database URL) — **never commit secrets**. Note: Add `API_URL=http://127.0.0.1:8000` to connect the UI to the Backend.

### 1. Setup

```bash
git clone [https://github.com/khoa26/ecommerce_analysis.git](https://github.com/khoa26/ecommerce_analysis.git)
...
### 3. Run the App (AI Dashboard)
To run the interactive dashboard, you must start the backend and frontend simultaneously in **two separate terminal windows**.

**Terminal 1: Start the Backend API (FastAPI)**
```bash
python src/api/main.py
```
*The API will be available at `http://localhost:8000`. You can check the Swagger UI at `/docs`.*
...
- **Microservices for GenAI:** Separating the Streamlit UI from the AI generation/execution logic using FastAPI prevents UI freezing and allows async background logging to run smoothly.
- **Human-in-the-loop is critical:** AI-generated code requires human validation. Implementing an edit-and-review step before execution prevents data manipulation errors and ensures trust in the system.
